...
export interface WasteContractScanResult {
  company: string | null
...
const PROMPT = `Przeanalizuj ten dokument umowy na odbiór odpadów (może się składać z kilku stron / zdjęć tego samego dokumentu) i wyciągnij dane.
Zwróć TYLKO poprawny JSON bez żadnego tekstu wokół niego, bez markdown, bez \`\`\`json.

Format odpowiedzi:
{
  "company": "nazwa firmy odbierającej odpady, z którą zawarto umowę (string lub null)",
  "signed_at": "data zawarcia/podpisania umowy w formacie YYYY-MM-DD (string lub null)",
  "notes": "dodatkowe uwagi np. numer umowy, zakres umowy, okres obowiązywania (string lub null)",
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result: WasteContractScanResult = JSON.parse(cleaned)
    return NextResponse.json(result)
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err)
    console.error('[scan-waste-contract] error:', msg)
